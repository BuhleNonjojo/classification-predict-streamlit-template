...
def main():
	...
	if selection == "About Us":
        ...
        # Resize the image to fit the browser window while maintaining aspect ratio
        width, height = image_k.size
        new_width = st.container.element().offsetWidth  # Get browser window width
        new_height = int(new_width * (height / width))  # Calculate proportional height

        image_k = image_k.resize((new_width, new_height), Image.ANTIALIAS)  # Resize with anti-aliasing

        # Display the image using streamlit and configure caption
        st.image(image_k, caption='Director: Buhle Nonjojo', use_column_width=True)
	...
	if selection == "Model Explainations":
		options = ['Logistic Regression','CatBoost Classifer','Decision Tree','Linear Support Vector Classifier','Multinomial Naives Bayes Classifier','Random Forest Classifier','XGBoost Classifier']
